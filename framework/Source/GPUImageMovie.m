...
- (dispatch_queue_t)getReaderQueue
{
    if (!_readerQueue) {
        _readerQueue = dispatch_queue_create([[NSString stringWithFormat:@"%@ GPUImageMovie reader queue", self] UTF8String], DISPATCH_QUEUE_SERIAL);
    }
...
- (void)startProcessing
{
    ...
    [inputAsset loadValuesAsynchronouslyForKeys:[NSArray arrayWithObject:@"tracks"] completionHandler: ^{
        dispatch_async([self getReaderQueue], ^{
            NSError *error = nil;
